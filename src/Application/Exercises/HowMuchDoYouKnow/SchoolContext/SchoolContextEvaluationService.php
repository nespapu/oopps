...
final class SchoolContextEvaluationService
{
    ...
    public function evaluate(array $payload, array $stepAnswer): StepEvaluation
    {
        ...
        if (!is_array($items) || !is_array($expectedItems) || !is_array($meta)) {
            throw new \InvalidArgumentException('Invalid school context payload structure.');
        }
        ...
        foreach ($items as $item) {
            ...
            foreach ($fieldConfig as $fieldKey => $config) {
                if ($config['evaluable'] !== true || !array_key_exists($fieldKey, $item)) {
                    continue;
                }
