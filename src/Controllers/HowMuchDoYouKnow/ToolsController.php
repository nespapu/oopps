...
final class ToolsController
{
    public function __construct(
        ...
        private readonly ToolsPayloadBuilder $payloadBuilder,
        private readonly ToolsEvaluationService $evaluationService,
    ...
    public function show(): void
    {
        ...
        $stepAnswer = $session->getStepAnswer(ExerciseStep::TOOLS);

        $evaluation = $session->getStepEvaluation(ExerciseStep::TOOLS);

        View::render('exercises/how-much-do-you-know/tools', [
            'payload' => $payload,
    ...
    public function evaluate(): void
    {
        ...
        $stepAnswer = $this->buildStepAnswerFromPost($payload, $_POST, ExerciseStep::TOOLS->value);

        $evaluation = $this->evaluationService->evaluate($payload, $stepAnswer);

        $session->setStepAnswer(ExerciseStep::TOOLS, $stepAnswer);

        $session->setStepEvaluation(ExerciseStep::TOOLS, $evaluation);

        $this->exerciseSessionStore->save($session);

        $this->redirector->redirect($this->paths->toolsStep($session->sessionId()));
    }
