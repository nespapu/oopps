...
final class BibliographyController
{
    public function __construct(
        ...
        private readonly BibliographyPayloadBuilder $payloadBuilder,
        private readonly BibliographyEvaluationService $evaluationService,
    ...
    public function show(): void
    {
        ...
        $stepAnswer = $session->getStepAnswer(ExerciseStep::BIBLIOGRAPHY);

        $evaluation = $session->getStepEvaluation(ExerciseStep::BIBLIOGRAPHY);

        View::render('exercises/how-much-do-you-know/bibliography', [
            'payload' => $payload,
    ...
    public function evaluate(): void
    {
        ...
        $stepAnswer = $this->buildStepAnswerFromPost($payload, $_POST, ExerciseStep::BIBLIOGRAPHY->value);

        $evaluation = $this->evaluationService->evaluate($payload, $stepAnswer);

        $session->setStepAnswer(ExerciseStep::BIBLIOGRAPHY, $stepAnswer);

        $session->setStepEvaluation(ExerciseStep::BIBLIOGRAPHY, $evaluation);

        $this->exerciseSessionStore->save($session);

        $this->redirector->redirect($this->paths->bibliographyStep($session->sessionId()));
    }
